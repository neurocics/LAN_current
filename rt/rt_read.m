...
% Francisco Zamorano
%
% 22.10.2024 fix an error when adding OTHER field in missing event. CHECK!! INCOMPLETE JET!!!! 
% 09.09.2024 fix read OTHER field in read RT structure 
...
for r = delind'  
    ...
    if ( r+(rp*dt) > length(del) ) ||( r+(rp*dt) <1 ) || ( del(r+(rp*dt)) == 1) || ( del(r+(rp*dt)) == 3)
        ...
        if exist('dataOTHER', 'var')
                for OT = fieldnames(dataOTHER)'
                    eval([ 'dataOTHERmiss.'  OT{1}  '(cmis)= dataOTHER.'  OT{1}  '(r);'])
                end
        end
        rp = -1;  % end, miss 
        cmis = cmis+1;
...
RT.nblock = 1;
    ...
    if exist('dataOTHERnomiss', 'var')
        RT.OTHER = dataOTHERnomiss;
        %for OT = fieldnames(dataOTHER)'
        %    eval([ 'dataOTHERnomiss'  OT{1}  '(c)= dataOTHER'  OT{1}  '(r);'])
        %end
    end
