...
% Asegurar que todos los valores sean reales y positivos para el log
realpow = real(avgfft(ch,2+mt:150));          % Eliminar parte imaginaria
realpow(realpow <= 0) = eps;                  % Reemplazar ceros o negativos con epsilon (valor mínimo positivo)

logpow = log10(realpow);

plot(f(2+mt:150), logpow, 'LineWidth', 2), hold on;
plot(f(2+mt:150), real(lslog(ch,2+mt:150)), 'Color', [0.5 0.5 0.5])
plot(f(2+mt:150), real(lilog(ch,2+mt:150)), 'Color', [0.5 0.5 0.5])
xlim([f(2+mt),f(150)]);
ylim([min(logpow)-1, max(logpow)+1])
