...
function [EEG ] = pop_loadeeg_lan(filename, filepath, range_chan, range_sweeps, range_typeeeg, range_response, datformat); 
% EEGLAB origial scripts
%
% comvertions per Pablo Billeke
% 10.11.2010
% command
EEG = [];
%command = '';
...
[EEG.data, accept, eegtype, rt, eegresp, namechan, EEG.pnts, EEG.trials, EEG.srate, EEG.xmin, EEG.xmax] = ...
    loadeeg_lan( fullFileName, range_chan, range_sweeps, range_typeeeg, 'all', 'all', range_response, datformat);
...
if any(rt)
    %EEG = pop_importepoch_lan( EEG, [rt(:)*1000 eegtype(:) accept(:) eegresp(:)], { 'RT' 'type' 'accept' 'response'}, {'RT'}, 1E-3, 0, 1);
else
    %EEG = pop_importepoch_lan( EEG, [eegtype(:) accept(:) eegresp(:)], { 'type' 'accept' 'response'}, { }, 1E-3, 0, 1);
end;    
