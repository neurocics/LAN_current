% axcopy_lan() - Copy a Matlab figure axis and its graphic objects to a new pop-up window 
...
function axcopy_lan(fig, command,GLAN)
...
for a=1:length(hndl)                    % make all axes visible
    if exist('command') ~= 1
        ...
        comstr = double(['copyaxis_lan(''' char(command_dbl) ''' , GLAN)']);
...
figure(fig);
if exist('command') ~= 1
    set(hndl(a),'ButtonDownFcn','copyaxis_lan');
else
    % set(hndl,'ButtonDownFcn',['copyaxis(''' command ''')']);
    comstr = double(['copyaxis_lan(''' char(command_dbl) ''')']);
