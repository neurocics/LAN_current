function [freq] = freq_wltconvol_lan(cfg,LAN, texto);
%
% Funcion de Fieldtrip 2009 adapatada para LAN
%            FIX CEIL 04.04.2013
%
...
if isfield(LAN, 'freq'),       freq   = LAN.freq;        end
%%%LAN
if nargin < 3
texto = plus_text();
end
...
sgnindx     = match_str_lan(data.label, cfg.channel);%%%LAN
...
numsmp = ceil(cfg.pad .* data.fsample); % VERIFICAR USO DE CEIL %%%%%%%%%%%%%%%%%%%%%%%%%%
...
freq.dimord     = dimord;
%freq.powspctrm  = permute(powspctrm,[2,1,3]);%%LAN
freq.freq       = cfg.foi;
freq.time       = cfg.toi;
...
if powflg
    ...
    if keep ==2
        for tt = 1:size(powspctrm,1)
            ...
            % save in file
            % LAN !!!
            if strcmp(cfg.ktt,'file')||strcmp(cfg.ktt,'file4chan')
                paso = what ;                
                filename = [LAN.name '_' LAN.cond '_' LAN.group '_' datestr(now,'HHMMSSFFF_dd_mm_yy') '.ldt'];
                filename = strrep(filename, ' ' , '_');
                t_powspctrm = freq.powspctrm;
                if tt >1
                m_powspctrm = mean(cat(4,freq.powspctrm{:}),4);
                save(filename,'t_powspctrm','m_powspctrm','-v7.3');
                else
                save([ filename],'t_powspctrm','-v7.3');
                end
    ...
    freq.powspctrm  = permute(single(powspctrm),[2,1,3]);    
    end
  % 
end
 ...
 %if powflg    
   % if keep ==2
   %     for tt = 1:size(powspctrm,1)
   %     freq.powspctrm{tt}  = permute(powspctrm(tt,:,:,:),[3,2,4,1]);
   %     end
   % else
