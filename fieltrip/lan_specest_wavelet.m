function [spectrum,freqoi,timeoi] = lan_specest_wavelet(dat, time, cfg)

% LAN_SPECEST_WAVELET picks up the equivalent fieldtrip function,
% FT_SPECEST_WAVELET, and replaces calls to other fieldtrip functions.
% Optional arguments should be provided within a single structure, unlike
...
if nargin >= 3
    if isfield(cfg, 'freqoi');     freqoi = cfg.freqoi; end;
    if isfield(cfg, 'timeoi');     timeoi = cfg.timeoi; end;
    if isfield(cfg, 'width');      width = cfg.width; end;
    if isfield(cfg, 'gwidth');     gwidth = cfg.gwidth; end;
    if isfield(cfg, 'pad');        pad = cfg.pad; end;
    if isfield(cfg, 'polyorder');  polyorder = cfg.polyorder; end;
    if isfield(cfg, 'fbopt');      fbopt = cfg.fbopt; end;
