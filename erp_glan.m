function erp_glan(GLAN,roi,comp,chh)
%    Remplazada por erp_plot.m ¡¡¡SIN MAS ACTUALIZACIONES!!!
% v.0.1.7
% erp_glan.m
%
...
% chh = 1 --> plotea significacia con correcciones
% chh = 0 --> plotea significacia sin correcciones
% 
% Pablo Billeke
% Rodrigo Henriquez
% Francisco Zamorano
%
%
% eg: erp_glan(glan,19,1,0)
% 
%  19.01.2011 CLOSE OUTDATE --> USE erp_plot.m 
%  13.11.2010 (PB) shading interpol per default
%  08.11.2010 (PB) incluye nuemro de cluster en grafico DIFF
%  16.08.2010 (PB) campo time: para poner manualmente el tiempo a plotear
%                           en grafico topografico, incluye interbalo de tiempo ej.: 0.3 0.5 (segundos)
%  21.06.2010 (PB) plotea promedio de grupos de electrodos
%  14.06.2010 (PB) boton de diferencia, y cluster de electrodo (depende de erp_stata.m  v.0.0.4p)
%  10.05.2010 (PB)
%  07.05.2010 (PB)
%  06.05.2010 (PB FZ)
%  16.04.2010 (PB RH)
...
warning('Remplazada por erp_plot.m ¡¡¡SIN MAS ACTUALIZACIONES!!!')
if nargin == 0
    edit erp_glan.m
    help erp_glan
    return
...
%if isfield(GLAN.erp, 'matdif') && ~isempty(GLAN.erp.matdif{ncomb}) 




if size(GLAN.erp.data,1)<10 %%%%%%%%%%%%%%%%%
figure('Visible','on','Position',[360,500,700,1000],'Color',[0 0 0 ],'MenuBar', 'none','DockControls','off');
hold on,

time(1) = 0;
time(2) = size(GLAN.erp.data{GLAN.erp.comp{1}(1)},2) /GLAN.srate;
time = time + GLAN.time(1);

ltime = linspace(time(1),time(2),size(GLAN.erp.data{GLAN.erp.comp{1}(1)},2));
cont = 0;
    if ifh
...
for c = GLAN.erp.comp{comp}
cont = cont + 1;

plot(ltime,GLAN.erp.data{c}(roi,:),'Color',color{cont},...
    'Interruptible','off');hold on;

if bll ==1
    text( (bl(1,1) ), (-1* cont ),[ GLAN.cond{c} ],'Color',color{cont} );  
...
n1 = GLAN.erp.comp{comp}(1);
n2 = GLAN.erp.comp{comp}(2);
...
if dif==0;
subplot(2,2,1)
[gx gy] = topoplot_eeglab(gp,GLAN.chanlocs,'emarker' , {'.','k',5,1}  ,'emarker2' , {rroi,'*','k'},'shading' ,'interp' , 'style' , 'map' );
colormap('jet');
caxis([c_axis]);hold on;
try title({GLAN.cond{n1};['at ' num2str(fix(X*1000)) ' ms'] }); end
...
hold off;
subplot(2,2,2)
[gx gy]=topoplot_eeglab(gp2,GLAN.chanlocs,'emarker' , {'.','k',5,1}  ,'emarker2' , {rroi,'*','k'} ,'shading' ,'interp' , 'style' , 'map' );
colormap('jet');
...
elseif dif ==1
    %%% DIF TOPOPLOT
subplot(2,2,1)
[gx gy] = topoplot_eeglab(gp-gp2,GLAN.chanlocs,'shading' ,'interp' , 'style' , 'map');
caxis(c_axis/2);hold on;
colormap('hot');
try title({[ GLAN.cond{n1} ' - ' GLAN.cond{n2} ] ;['at ' num2str(fix(X*1000)) ' ms'] });end
%
plot(gy(rroi),gx(rroi),...
                'ko','LineStyle','none',...
                'MarkerFaceColor',[1 0.8 0.8],...'g',...
...
subplot(2,2,2)
[gx gy] = topoplot_eeglab(zeros(size(gp)),GLAN.chanlocs,'style'  ,'blank','electrodes','labelpoint','emarker',{'.','k',[],0.5});  
caxis(c_axis/2);hold on;
...
linspace(time(1),time(2),size(GLAN.erp.data{GLAN.erp.comp{1}(1)},2));
